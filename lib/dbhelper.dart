...
class DBhelper {
  ...
  Future<Database> openDb () async{
    ...
    return await openDatabase (path, version: 1, onCreate:(db, version) {
      ...
      db.execute(
          "create table $userdetails ($userid integer primary key autoincrement,"
              "$usermobile text, "
              "$useremail text, "
              "$userpassword text, "
              "$username text)");
  ...
  void delData (int id) async {
    var myDB = await openDb();
    await myDB.delete(dialogueTable, where: '$columnId = ?', whereArgs: [id.toString()]);

  }

  //For user table

  void signupadddata (String name, String email, String mobile, String password) async {
    var myDB = await openDb();
    myDB.insert (userdetails, {
  ...
  Future<bool> isemailexist (String email) async{
    var myDB = await openDb();
    var data= await myDB.query(userdetails, where: '$useremail = ?', whereArgs: [email]);
    if(data.isNotEmpty) {
      return true;  // user cannot login
    } else {
      return false; // user can login
    }
  ...
  Future<bool> Authentication (String email, String password) async {
    var myDB = await openDb();
    var data = await myDB.query (userdetails, where: '$useremail = ? and $userpassword = ?', whereArgs: [email, password]);
    if (data.isNotEmpty) {
      return true;  // you can login
    } else {
